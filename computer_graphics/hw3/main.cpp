...
{
public:
    static void sirpienskiCarpet(double size, int level, CPunct p, CVector v)
    {
        ...
        if (level > 0)
        {
            ...
            sirpienskiCarpet(size / 3, level - 1, CPunct(x, y), v);

            sirpienskiCarpet(size / 3, level - 1, CPunct(x + size / 3, y), v);

            sirpienskiCarpet(size / 3, level - 1, CPunct(x + 2 * size / 3.0, y), v);

            sirpienskiCarpet(size / 3, level - 1, CPunct(x, y + size / 3.0), v);

            CVector v2(0, -1);
            CPunct p2(x + size / 3.0, y + size / 3.0);

            for (int i = 0; i < 4; ++i)
            {
                v2.rotatie(90);
                v2.deseneaza(p2, size / 3.0);
                p2 = v2.getDest(p2, size / 3.0);
            }

            sirpienskiCarpet(size / 3, level - 1, CPunct(x + 2 * size / 3.0, y + size / 3.0), v);

            sirpienskiCarpet(size / 3, level - 1, CPunct(x, y + 2 * size / 3.0), v);

            sirpienskiCarpet(size / 3, level - 1, CPunct(x + size / 3.0, y + 2 * size / 3.0), v);

            sirpienskiCarpet(size / 3, level - 1, CPunct(x + 2 * size / 3.0, y + 2 * size / 3.0), v);

        }
    }

    void show(double size, int level)
    {
        CVector v(.5, .5);
        CPunct p(.0, .0);

        sirpienskiCarpet(size, level, p, v);
    }

};


class UndefinedFractal1
{
public:
    static void undefinedFractal1(double size, int level, CPunct p, CVector v)
    {
    ...
    void show(double size, int level)
    {
        ...
        undefinedFractal1(size, level, p, v);
...
// It is actually called sierpinski arrowhead curve...
// https://en.wikipedia.org/wiki/Sierpi%C5%84ski_arrowhead_curve
class UndefinedFractal2
{
public:
    static void sierpinskiArrowheadCurve(int order, double length, CPunct p, CVector v)
    {
    ...
    static void curve(int order, double length, double angle, CPunct p, CVector v)
    {
        // in mare am luat algoritmul de pe wikipedia si l-am adaptat...
        if (order == 0)
        {
        ...
        {
            v.rotatie(angle);
            curve(order - 1, length / 2, -angle, p, v);

            p = v.getDest(p, length / 2);
            v.rotatie(-angle);
            curve(order - 1, length / 2, angle, p, v);
    ...
    void show(double size, int level)
    {
        ...
        sierpinskiArrowheadCurve(level, size, p, v);
    }
};
        ...
        for (double x = xmin; x <= xmax; x+=RX_JF)
            for (double y = ymin; y <= ymax; y+=RY_JF) {
                ...
                if (r == 0) {
                    ...
                    glColor3f(1.0 * 1 / nriter, 0.1 * 1 / nriter, 0.1 * 1 / nriter);
                    glVertex3d(x / 2.0, y / 2.0, 0);
                } else if (r == 1) {
                    glColor3f(1.0 * 0.5 / nriter, 0.1 * 0.5 / nriter, 0.1 * 0.5 / nriter);
                    glVertex3d(x / 2.0, y / 2.0, 0);
