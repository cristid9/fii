...
void Display8() {
    ...
    glColor3f(0.1f, 0.0f, 1.0f); // (0.5, 0, 1) is half red and full blue, giving dark purple.
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    glBegin(GL_POLYGON);
    for(int i = 0; i < 6; ++i) {
        glVertex2f(cos(i/6.0*2*M_PI)/1.2,
                   sin(i/6.0*2*M_PI)/1.2);
    }
    glEnd();

    glColor3f(1.0f, 0.0f, 0.0f); // (0.5, 0, 1) is half red and full blue, giving dark purple.
    glBegin(GL_POLYGON);
        for(int i = 0; i < 6; ++i) {
            glVertex2f(cos(i/6.0*2*M_PI)/1.5,
                       sin(i/6.0*2*M_PI)/1.5);
        }
    glEnd();

    glColor3f(1.0f, 1.0f, 1.0f); // (0.5, 0, 1) is half red and full blue, giving dark purple.
    glBegin(GL_POLYGON);
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    for(int i = 0; i < 6; ++i) {
        glVertex2f(cos(i/6.0*2*M_PI)/1.6,
                   sin(i/6.0*2*M_PI)/1.6);
    }
    glEnd();
}
